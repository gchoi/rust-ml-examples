...
fn main() -> Result<()> {
    ...
    let src = PathBuf::from("src/kernels/layernorm_kernels.ptx");
    let dst = PathBuf::from(&out_dir).join("layernorm_kernels.ptx");
    fs::copy(&src, &dst).unwrap();

    // Tell rustc where to find it
    println!("cargo:rustc-env=LAYERNORM_PTX_PATH={}", dst.display());
