...
struct SegmentationArgs {
    #[arg(
        long,
        help = "name of the web-serving hub model",
    ...
    #[arg(
        ...
        default_value = "./assets/labels.json"
    )]
    label_path: PathBuf,
...
fn get_vb_and_config(
    ...
    println!("loading model {model_name} via web-serving hub");
    let api = hf_hub::api::sync::Api::new()?;
    let api = api.model(model_name.clone());
    let model_file = api.get("model.safetensors")?;
    println!("model {model_name} downloaded and loaded");
    let vb =
        unsafe { VarBuilder::from_mmaped_safetensors(&[model_file], candle_core::DType::F32, device)? };
